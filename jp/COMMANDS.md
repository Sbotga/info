...
- `/song jacket <song>` - 指定された曲のジャケ絵を見る。
    - `<song>`: どの曲のジャケ絵を見るか。
        - 必須。
- `/song skillorder <song>` - 指定された楽曲のスキルの発動の順番を見る。
    - `<song>`: どの曲を見るか。
      - 必須。
- `/song constant <song> [difficulty]` - 指定された楽曲の譜面定数(非公式)を見る。View the constant (community-rated difficulty) of a specified song.
    - `<song>`: どの曲の定数を見るか。
      - 必須。
