...
- `/ranked view [rank] [region]` - 現在のシーズンのTOP100のプレイヤーの統計を見る。
    ...
    - `[region]`: どのリージョンを見るか。
        - 任意、空欄の場合デフォルトのリージョンになる(設定されてない場合グローバル)。
...
- `/event schedule [region]` - 今後のイベントのスケジュールを見る。
    - `[region]`: どのリージョンのイベントを見るか。
        - 任意、空欄の場合デフォルトのリージョンになる(設定されてない場合グローバル)。
- `/event border [region]` - 現在のイベントのボーダーを見る。
    - `[region]`: どのリージョンのイベントを見るか。
        - 任意、空欄の場合デフォルトのリージョンになる(設定されてない場合グローバル)。
### クイズ用コマンド
...
- `/song progress <song> [region]` - 指定された楽曲の進歩を見る。
    ...
    - `[region]`: どの連携されたアカウントを見るか。(ex.グローバルの曲の進歩)
        - 任意、空欄の場合デフォルトのリージョンになる(設定されてない場合グローバル)。
- `/song difficulty <level>` - 指定された難易度のすべての楽曲を見る。
    - `<level>`: どのレベルの曲を見るか。
      - 必須。
### ユーザーデータ・設定コマンド
> [!IMPORTANT] 
> 高度なコマンドを使うのと何個かのコマンドの情報は[LINKING](LINKING.md)で見つかります。
> "なぜ必要か"も書いてあります。
